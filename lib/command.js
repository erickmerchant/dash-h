...
module.exports = class {
  ...
  help () {
    ...
    output.push(format`${chalk.green('Usage:') + ' ' + this.name + ' ' + (this.optionsParameters.size ? [...this.optionsParameters]
...
.map(([key, arg]) => {
  ...
  return '[' + (arg.key.length < 2 ? '-' : '--') + arg.key + ']'
    ...
    if (paramCount) {
      output.push(format`${chalk.green('Parameters:')}
...
${[...this.optionsParameters].map(([key, arg]) => {
  if (Number.isInteger(key)) {
    ...
    if (optCount) {
      output.push(format`${chalk.green('Options:')}
